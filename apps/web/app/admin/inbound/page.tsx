...
export default async function AdminInboundPage() {
  ...
  return (
    <div className="flex flex-col gap-5">
      <div>
        <h1 className="text-3xl font-bold text-snappeal-navy">Inbound mail</h1>
      ...
      <div className="overflow-hidden rounded-2xl bg-white border border-snappeal-border">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-snappeal-border">
            {rows.length === 0 && (
              <tr>
                <td colSpan={5} className="px-4 py-8 text-center text-snappeal-muted">
                  No inbound messages yet.
                </td>
              </tr>
            )}
            {rows.map((m) => (
              <tr key={m.id} className="hover:bg-snappeal-bg/40 transition">
                <td className="px-4 py-3 text-snappeal-navy font-semibold truncate max-w-[280px]">{m.subject ?? "(no subject)"}</td>
                <td className="px-4 py-3 text-[11px] text-snappeal-muted">{m.fromAddr}</td>
                ...
                <td className="px-4 py-3 font-mono text-[11px]">
                  {m.appealId ? (
                    <Link href={`/admin/appeals/${m.appealId}`} className="text-snappeal-primary hover:underline">
                      {m.appealId}
                    </Link>
                  ) : (
                    "—"
