...
export default function SignInPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center text-sm text-parkingrabbit-muted">Loading…</div>}>
      <SignInInner />
    </Suspense>
  );
}
    ...
    <AuthShell title="Welcome back" subtitle="Sign in to ParkingRabbit to sync your tickets across devices.">
      ...
      <form onSubmit={onSubmit} className="flex flex-col gap-3">
        ...
        <p className="text-xs text-parkingrabbit-muted text-center mt-2">
          No account yet?{" "}
          <Link href={`/sign-up?next=${encodeURIComponent(next)}`} className="font-semibold text-parkingrabbit-primary">
            Create one
          </Link>
        </p>
        <p className="text-xs text-parkingrabbit-muted text-center">
          Or{" "}
          <Link href="/app" className="font-semibold text-parkingrabbit-primary">
            continue as guest
          </Link>
        </p>
      </form>
    ...
    <div className="min-h-screen bg-parkingrabbit-bg flex flex-col items-center justify-center px-5 py-12">
      <div className="w-full max-w-sm">
        <Link href="/app" className="flex items-center gap-2.5 mb-6 justify-center">
          ...
          <span className="text-xl font-bold text-parkingrabbit-navy tracking-tight">ParkingRabbit</span>
        </Link>
        <div className="rounded-3xl bg-white border border-parkingrabbit-border p-6">
          <h1 className="text-2xl font-bold text-parkingrabbit-navy">{title}</h1>
          <p className="text-sm text-parkingrabbit-muted mt-1">{subtitle}</p>
