...
export function CouncilPickerSheet({
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-parkingrabbit-navy/40 backdrop-blur-sm p-0 sm:p-4"
      onClick={onClose}
      role="dialog"
      aria-modal="true"
    ...
    >
      <div
        ...
        className="w-full sm:max-w-md bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl border border-parkingrabbit-border max-h-[85vh] flex flex-col"
      >
        <div className="px-5 pt-4 pb-3 flex items-start gap-3 border-b border-parkingrabbit-border">
          ...
          <button
            ...
            className="size-8 -mt-1 -mr-1 rounded-full text-parkingrabbit-muted hover:bg-parkingrabbit-bg flex items-center justify-center shrink-0"
          >
  ...
  return (
    <button
      ...
      className={`relative rounded-2xl border bg-white p-2.5 flex flex-col items-center text-center gap-2 transition active:scale-[0.97] ${
        selected
          ? "border-parkingrabbit-primary ring-2 ring-parkingrabbit-primary/30 shadow-sm"
          : "border-parkingrabbit-border hover:border-parkingrabbit-primary/50"
      }`}
    >
      {selected && (
        <span className="absolute top-1.5 right-1.5 size-5 rounded-full bg-parkingrabbit-primary text-white flex items-center justify-center shadow">
          <Check className="size-3" strokeWidth={3} />
        </span>
      )}
      <span
        className="w-full aspect-square rounded-xl border border-parkingrabbit-border flex items-center justify-center overflow-hidden"
      ...
      >
        {council.logoUrl ? (
          ...
          <Building2 className="size-7 text-parkingrabbit-muted" strokeWidth={1.75} />
        )}
      </span>
      <span className="text-[11.5px] font-semibold text-parkingrabbit-navy leading-tight line-clamp-2">
        {council.name}
      </span>
    </button>
  );
