...
export function InboundClassifierSandbox() {
  ...
  return (
    <section className="rounded-2xl bg-white border border-snappeal-border p-5 flex flex-col gap-3">
      <div>
        <p className="text-sm font-bold text-snappeal-navy">Classifier sandbox</p>
      ...
      <input
        ...
        className="bg-snappeal-bg/50 border border-snappeal-border rounded-xl px-3 py-2 text-sm outline-none focus:border-snappeal-primary"
      />
      <textarea
        value={body}
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          className="inline-flex items-center gap-2 rounded-2xl bg-snappeal-action text-white font-semibold px-4 py-2.5 text-sm shadow-md hover:bg-snappeal-action-600 transition disabled:opacity-60"
        >
          {busy ? <Loader2 className="size-4 animate-spin" /> : <Sparkles className="size-4" />}
          Classify
        </button>
        {cost != null && (
          <span className="text-[11px] text-snappeal-muted">cost ${cost.toFixed(4)}</span>
        )}
      </div>
      {error && (
      ...
      {result && (
        <div className="rounded-xl bg-snappeal-bg/40 border border-snappeal-border p-3 flex flex-col gap-2">
          ...
          <p className="text-xs text-snappeal-navy leading-relaxed">{result.reasoning}</p>
        </div>
      )}
    </section>
  );
