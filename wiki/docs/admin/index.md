...
Operational documentation for the live ParkingRabbit admin backend at `/admin/*`. Gated by `users.role = 'admin'` — guests are bounced to `/sign-in?next=/admin`, signed-in non-admins to `/app?notAdmin=1`. **16 pages live** (added `/admin/notifications` and per-user notification-prefs editor in v0.3.9; the appeal-detail page now surfaces a per-stage `ai_calls` cost breakdown). Full route inventory + API surface in [architecture/admin.md](../architecture/admin.md); this section is the operator-facing runbook.
...
- [Managing subscriptions](managing-subscriptions.md) — Care Plan (£9.99/mo) schema + waitlist surface + Stripe scaffold. **Scaffold only — webhook + admin CRUD pending.**
