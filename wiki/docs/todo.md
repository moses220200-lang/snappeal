...
### UK IPO trademark search + filing — "Snappeal"
...
### Transactional email — `appeals.snappeal.ai` MX setup
- Per-user aliases (`<user-id>@appeals.snappeal.ai`) for the council email-submission path + inbound reply parsing. Provider candidates: Resend, Postmark, AWS SES. DNS records: MX, SPF, DKIM, DMARC.
...
- Top-5 boroughs by volume should hear about Snappeal directly from us before they hear about it via PCN submission volume changes. Tonally: collaborator not adversary. Risks.md R8 mitigation.
